...
import os
import csv 
folder_path = r"D:\CS\Grad\Dataset\\excel 2"
...
for file in excels:
    ...
    # Exclude the first row (header) and calculate the median for each column
    column_medians = df.iloc[1:, :].median(axis=0)
    row.append (column_medians)
    print(f"Median values for each column in {file}:\n{column_medians}\n")
...
with open( "e2median.csv", 'w', newline='') as csvfile:
